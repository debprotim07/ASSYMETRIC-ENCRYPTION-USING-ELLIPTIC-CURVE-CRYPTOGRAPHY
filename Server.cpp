...
FiniteFieldElement<P> a() const { return a_; } FiniteFieldElement<P> b() const { return b_; } 
template<int T> 
...
cout << "G = " << G << ", order(G) is " << G.Order() << "\n\n"; int a = irand(1,myEllipticCurve.Degree()-1); int m1,m2,tot,a1,b1,c,d; 
...
cout<<"Shaswat's private key is a = "<<a<<endl; ec_t::Point Pa = a*G; // public key 
...
ec_t::ffe_t m1d = c1/Pk.x(); ec_t::ffe_t m2d = c2/Pk.y(); Pk = j*Pa; 
...
// Accept the data packet from client and verification connfd = accept(sockfd, (SA*)&cli,(socklen_t*) &len); if (connfd < 0) { 
