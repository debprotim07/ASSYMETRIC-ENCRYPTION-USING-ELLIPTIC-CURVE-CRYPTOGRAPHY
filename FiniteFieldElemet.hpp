...
// helper functions namespace detail 
 
{ 
 
//From Knuth; Extended GCD gives g = a*u + b*v int EGCD(int a, int b, int& u, int &v) 
...
x = x % n; // % is the remainder function, 0 <= x % n < |n| int u,v,g,z; 
...
// x and n have to be relative prime for there to exist an x^-1 mod n z = 0; 
...
// assign from integer FiniteFieldElement& operator=(int i) 
...
// ostream handler template<int T> 
