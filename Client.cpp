...
// bit is set; acc = 2^(ij)*acc addDouble(i-j,acc); 
...
// copy ctor Point(const Point& rhs) 
...
// access x component as element of Fp ffe_t x() const { return x_; } 
 
// access y component as element of Fp ffe_t y() const { return y_; } 
...
int Degree() const { return P; } FiniteFieldElement<P> a() const { return a_; } 
...
m_table_t m_table_; FiniteFieldElement<P> a_; 
...
equation // true if the table has been 
calculated 
}; 
...
cout << "G = " << G << ", order(G) is " << G.Order() << "\n\n"; int a = irand(1,myEllipticCurve.Degree()-1); ec_t::Point Pa = a*G; // public key 
...
int b = irand(1,myEllipticCurve.Degree()-1);; cout<<"Amitesh's private key is b = "<<b<<endl; ec_t::Point Pb = b*G; // public key 
...
cout << "Decrypted message = (" << m1d << ", " << m2d << ")" << endl; Pk = j*Pa; 
 
m1d = c1/Pk.x(); m2d = c2/Pk.y(); 
